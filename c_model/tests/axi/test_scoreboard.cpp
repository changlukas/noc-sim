...
TEST(Scoreboard, NoUpdateOnDecerr) {
  ...
  sb.handle_write_completed(
      axi::WriteResult{0x100, {0xAB, 0xCD, 0xEF, 0x12}, axi::Resp::DECERR, 1, 1},
...
TEST(Scoreboard, MismatchDetected) {
  axi::Scoreboard sb;
  sb.handle_write_completed(
      axi::WriteResult{0x200, {0xAB, 0xCD, 0xEF, 0x12}, axi::Resp::OKAY, 1, 1},
      std::vector<uint8_t>{0xAB, 0xCD, 0xEF, 0x12});
...
TEST(Scoreboard, MatchPassesSilent) {
  axi::Scoreboard sb;
  sb.handle_write_completed(
      axi::WriteResult{0x300, {0xDE, 0xAD, 0xBE, 0xEF}, axi::Resp::OKAY, 1, 1},
