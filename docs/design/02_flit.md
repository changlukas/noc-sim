...
`src_id` 與 `dst_id` 寬度為 `X_WIDTH + Y_WIDTH`（預設 8 bits），編碼 `[X_WIDTH-1:0]=x`、`[X_WIDTH+Y_WIDTH-1:X_WIDTH]=y`。每個 tile 對應一個 NI (network interface)，`(x, y)` 為其唯一識別。多 IP 共享一個 tile 的場景由 NI 上游 AXI crossbar 處理（per `04_network_interface.md`），不在 flit format scope。
...
| `ROUTE_PAR_WIDTH` | 1 | Even parity over `{dst_id, last}` (9 bits at default). Always-on, checked at every router hop. Protects routing-critical fields: DST ID parity generated at NMU/NSU and checked at the router. `src_id` is covered by end-to-end `flit_ecc` (whole-flit SECDED) at the destination NI sink — no need to duplicate in per-hop parity |
...
(v0.3.0 為 48/352/400。Header 從 48→54 因加 `route_par` (1b) + `flit_ecc` (10b) 並回收 3-bit MSB `rsvd` 槽位，淨 +6。pre-(δ) 中間版本曾為 56 bits 含 `port_id` (2b) 欄位；改 single-NI-per-tile 模型後 `port_id` 移除，回到 54。)
...
| route_par | `ROUTE_PAR_WIDTH` | [26] | routing | Even parity over `{dst_id, last}` (9 bits at default); checked at every router hop. Covers routing-critical fields (destination ID + last-flit indicator) |
...
每個 tile 對應一個 NI (network interface)，`(x, y)` 為其唯一識別。tile 內若有多個 IP（CPU + DMA + memory controller + accelerator 等），由 NI 上游 AXI crossbar mux 到單一 NI 接口；不同 IP 的識別由 AXI ID 區分，不在 flit header 出現。
...
Always-on even parity over routing-critical fields. Covers destination ID and last-flit indicator.
...
| Property | Value |
|----------|-------|
| Coverage | `dst_id` (8b) + `last` (1b) = 9 bits at default. Covers routing-critical fields (destination ID + last-flit indicator) |
...
- **Two-Layer Data Integrity** — `flit_ecc` (`FLIT_ECC_WIDTH`), `route_par` (`ROUTE_PAR_WIDTH`)
  - **Layer 1 routing protection**: 1-bit even parity over `{dst_id, last}` (9 bits at default); checked at every router hop; fatal interrupt on mismatch
