...
- **cg_protocol_rule_hits** — one cover-property per rule ID in protocol_rules.md. Final count: **139 rule IDs** post-A4.5 (across all sections; canonical `grep -c '^| AXI4\\|^| NOC\\|^| NI\\|^| AXI4LITE' protocol_rules.md`). One cover bin per rule. Detailed breakdown: 5 RST + 3 CDC + 11 AW + 7 W + 6 B + 10 AR + 9 R + 4 XCH + 5 RoB + 3 Exclusive + 1 Timeout + 2 AXI parity + 5 NoC handshake (incl. credit) + 13 NoC flit + 4 ECC + 4 VC + 2 Width-conv + 24 AXI4-Lite CSR + 20 CFG + 1 IRQ = 139. (A4.5 wave removed `NOC_FLIT_HDR_PORT_ID_VALID` and `NOC_FLIT_RSP_PORT_ID_INHERIT` for FlooNoC alignment; NoC flit count 15→13.)

D3 coverage closure goal: 100% bin hits on every covergroup.

## ABV / FPV strategy

**ABV** — every FAIL-severity row in protocol_rules.md gets one SVA `assert property` in the testbench. Post-A4.5 canonical count: **129 FAIL-severity assertions** + **10 RECOMMEND cover-properties** (RECOMMEND family: `NI_RST_PARTIAL`, `AXI4_MST_AW_AWCACHE_STABLE`, `AXI4_MST_AR_ARCACHE_STABLE`, `NOC_FLIT_HDR_RSVD_IGNORE_RX`, `NOC_VC_PARTITION`, `AXI4LITE_SLV_RO_WRITE_IGNORED`, `NI_CFG_RESPONSE_DELAY_AXI`, `NI_CFG_RESPONSE_DELAY_NOC`, `NI_CFG_INJECT_ECC_ERROR`, `NI_CFG_RESPONSE_FAULT`). Total ABV library size: 139 properties.
...
- route_par parity — formally verify XOR-reduction over `{src_id, dst_id}` and the drop-on-mismatch behaviour.
