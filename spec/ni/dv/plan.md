...
14. NMU-side software quiesce flow (`QUIESCE_CTRL` / `QUIESCE_STATUS`; drain correctness; NMU-only scope; liveness bound under timeout-driven drain).
...
| TP45 | Quiesce liveness under stuck slave | Force one slave to never respond on one outstanding transaction (e.g., via `set_response_delay_axi(∞, ∞)` on a remote NSU); assert `quiesce_req=1` while that transaction is in flight. Verify `quiesce_idle` eventually asserts after the stuck transaction times out at `TXN_TIMEOUT` cycles (drains via timeout-induced SLVERR per `AXI4_MST_TIMEOUT_SLVERR`). Verify total wait ≤ `MAX_TXNS × TXN_TIMEOUT` aclk cycles (default 320 000). Verify subsequent reconfig + resume works correctly. | NI_CFG_QUIESCE_LIVENESS; AXI4_MST_TIMEOUT_SLVERR |
...
- **cg_axi_parity** — bins across (no error, NMU-side awaddr-byte parity error, NMU-side araddr-byte parity error, NMU-side wdata-byte parity error, NSU-side rdata-byte parity error). All check paths log to `ERR_STATUS[3]` + `AXI_PARITY_ERR_CNT`; AXI rresp/bresp unchanged. Plus `axi_rdata_par_o` regeneration bins (NMU R-direction): no-error / 1-bit-flit_ecc-corrected / 2-bit-flit_ecc-uncorrectable cases — verify parity reflects on-the-wire data, not pre-correction (per `AXI4_MST_PARITY_GEN_R`).
...
- **cg_quiesce** — bins across (idle [no quiesce], drain-in-progress [quiesce_req=1, pending>0], drain-complete [quiesce_req=1, pending=0, idle=1], resume [quiesce_req=0 after idle]); plus drain-via-timeout bin (any outstanding transaction released by `TXN_TIMEOUT` while quiesce_req=1).
...
- **cg_protocol_rule_hits** — one cover-property per rule ID in protocol_rules.md. Final count: **140 rule IDs** post-A4.6 (across all sections; canonical `grep -c '^| AXI4\\|^| NOC\\|^| NI\\|^| AXI4LITE' protocol_rules.md`). One cover bin per rule. Detailed breakdown: 5 RST + 3 CDC + 11 AW + 7 W + 6 B + 10 AR + 9 R + 4 XCH + 5 RoB + 3 Exclusive + 1 Timeout + 3 AXI parity + 5 NoC handshake (incl. credit) + 13 NoC flit + 4 ECC + 4 VC + 2 Width-conv + 24 AXI4-Lite CSR + 20 CFG + 1 IRQ = 140. (A4.5 removed 2 port_id rules for FlooNoC alignment, NoC flit 15→13. A4.6 added `AXI4_MST_PARITY_GEN_R` for R-direction parity regen alignment with AMD pg313 §Parity, AXI parity 2→3.)

D3 coverage closure goal: 100% bin hits on every covergroup.

## ABV / FPV strategy

**ABV** — every FAIL-severity row in protocol_rules.md gets one SVA `assert property` in the testbench. Post-A4.6 canonical count: **130 FAIL-severity assertions** + **10 RECOMMEND cover-properties** (RECOMMEND family: `NI_RST_PARTIAL`, `AXI4_MST_AW_AWCACHE_STABLE`, `AXI4_MST_AR_ARCACHE_STABLE`, `NOC_FLIT_HDR_RSVD_IGNORE_RX`, `NOC_VC_PARTITION`, `AXI4LITE_SLV_RO_WRITE_IGNORED`, `NI_CFG_RESPONSE_DELAY_AXI`, `NI_CFG_RESPONSE_DELAY_NOC`, `NI_CFG_INJECT_ECC_ERROR`, `NI_CFG_RESPONSE_FAULT`). Total ABV library size: 140 properties.
...
- Software quiesce liveness — formally verify that `QUIESCE_STATUS.quiesce_idle` eventually asserts whenever `QUIESCE_CTRL.quiesce_req=1` is held continuously; drain bounded by `MAX_TXNS × TXN_TIMEOUT` aclk cycles via the timeout path. Piggy-backs on the outstanding-transaction-timeout proof above. Per `protocol_rules.md` `NI_CFG_QUIESCE_LIVENESS`.
