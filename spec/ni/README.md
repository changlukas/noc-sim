...
- **Two-layer NoC integrity** — 1-bit per-hop routing parity (`route_par`, covers `{dst_id, last}`) checked at every router output and NI sink; whole-flit SECDED (`flit_ecc`) generated by source NI and validated **only** at destination NI sink (routers do not check). Aligns with AMD pg313 §Data Integrity.
...
The NI implements AXI4 (ARM IHI 0022) on the host side. Architectural conventions (credit-based flow control, two-layer ECC, NMU/NSU role split) align with AMD pg313 Versal NoC where noted in the detailed spec. The register set, QoS / Probe / Error CSR design, and the flit format are this IP's own conventions; the flit format is internal to this NoC and not interoperable with external networks.
