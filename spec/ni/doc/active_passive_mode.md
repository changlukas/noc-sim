...
- **Default**: `ACTIVE`
- **Granularity**: per-NI (NMU and NSU switch together). TODO(designer): consider whether independent NMU/NSU mode switching is needed for use cases like "active master, passive slave-side observation" (no issue yet — design question deferred to v0.4.0; current default of single bfm_mode covers all known testbench setups in §Common testbench setups). Default current behavior: single bfm_mode applies to both units.
