...
| AXI4_MST_W_WSTRB_NARROW | WVALID is HIGH and AWSIZE < log2(NOC_DATA_WIDTH/8) | WSTRB bits outside the addressed lanes (per AWSIZE) must be 0. | FAIL | AXI4_ERRM_WSTRB_NARROW (unverified) |
...
| NOC_FLIT_HDR_ROUTE_PAR_CHECK | A router output port or an NI sink receives a flit | Recompute `route_par` over the received `{dst_id, last}` and compare to header `route_par` field. Mismatch → drop the flit (do NOT forward; misrouting or wormhole-arbiter-disruption risk), increment `ROUTE_PAR_ERR_CNT` (saturating, per `registers.md`), set `ERR_STATUS[1] route_par_err`, capture `LAST_ERR_INFO` if no prior un-cleared error is sticky. The dropped flit's originating AXI transaction will hang silently at the master — no AXI-rresp synthesis path on routing faults in v0.4.0. Software detects via the IRQ + counter and handles recovery externally. | FAIL | (none) |
| NOC_FLIT_HDR_FLIT_ECC_GEN | NMU or NSU injecting a flit on `noc_req_o` / `noc_rsp_o` | Compute SECDED Hsiao code over the entire flit (`flit[FLIT_WIDTH-1:0]`) excluding the `flit_ecc` field bits, and place the resulting `FLIT_ECC_WIDTH`-bit syndrome (default 10) in the flit-header `flit_ecc` field. Width parameter constraint per `signal_interface.md` §Parameters under `FLIT_ECC_WIDTH`. **Parity-check matrix**: BFM and RTL MUST consume the same H-matrix. Recommended source: generate via OpenTitan `util/design/secded_gen.py` (Hsiao code family) for the `(FLIT_DATA_WIDTH, FLIT_ECC_WIDTH)` tuple and check the generated SystemVerilog header into the project as the single source of truth (both teams link against it). Alternative: FlooNoC `floo_secded_*.sv` family. Wire-equivalence between BFM and RTL is conditional on identical H-matrix bits. **Designer-confirmed (D1→D2 ambiguity triage, 2026-05-10): shared H-matrix source mandatory.** | FAIL | (none) |
...
| NOC_VC_PARTITION | NMU / NSU instantiated with `NUM_VC ∈ {1, 2, 4, 8}` (validated values per ToO §"VC partition policy") | Each physical channel's `NUM_VC` virtual channels SHOULD be partitioned by QoS tier per the ToO recommended table (e.g., `NUM_VC=2` → 2 QoS tiers per physical channel). The two physical channels (`noc_req`, `noc_rsp`) carry the same partition independently — VC numbering does not cross channels. Exact qos-to-VC mapping is integrator-configurable; the default mapping is what the BFM models per `NOC_VC_MAPPING_HYBRID_RW_QOS`. Non-recommended NUM_VC values (e.g., `NUM_VC=3`) are not validated; integrator's responsibility to verify deadlock-freedom. | RECOMMEND | (none) |
| NOC_VC_MAPPING_HYBRID_RW_QOS | NMU / NSU constructs an outbound flit when `NUM_VC > 1` | The flit's `vc_id` is assigned by QoS-tier mapping within the flit's target physical channel. Request flits map among `NUM_VC` VCs on `noc_req`; response flits map among `NUM_VC` VCs on `noc_rsp`. The two physical channels' VC pools are independent — no shared numbering. Mapping is a function of `qos` only. **Default mapping**: divide the 16-value `qos[3:0]` space into `NUM_VC` equal-sized tiers by magnitude; the tier number becomes `vc_id`. High-qos flits land on high-numbered VCs (monotonic, priority-preserving). Concrete: NUM_VC=2 → qos<8 → VC[0], qos≥8 → VC[1]; NUM_VC=4 → qos∈[4k, 4k+3] → VC[k]; NUM_VC=8 → qos∈[2k, 2k+1] → VC[k]. Compact form: `vc_id = qos >> (4 - log2(NUM_VC))`. The BFM models this default; RTL implementations using a different magnitude-to-VC function MUST publish the choice as a parameter or build-time constant accessible to the BFM counterpart for wire-equivalence verification. **Anti-patterns rejected**: (a) `vc_id = qos % NUM_VC` (modulo) scatters same-priority flits across VCs, defeats priority isolation; (b) low-bit extract `qos[log2(NUM_VC)-1:0]` has the same scattering problem. Policy is fixed at design time, no runtime alternative. The "R/W" in this rule's name reflects that request and response flits see different physical-channel VC pools, not that R/W is part of the qos→vc_id function within a given pool. Cycle-level VC arbitration in the network switch is a separate function (NPS-side, per AMD pg313 §Virtual Channel Arbitration), out of NI scope. | FAIL | (none) |
...
These rules govern Upsize and Downsize behaviour performed by the external Width Bridge at the AXI ↔ NI boundary (structural divergence from AMD — see ToO §"Data Width Conversion (Upsize / Downsize)"), when `AXI_DATA_WIDTH` differs from `NOC_DATA_WIDTH`. Per ToO §"Data Width Conversion (Upsize / Downsize)" and §"Over-fetch and WSTRB regeneration".

**Scope — Width Bridge spec (out of basic-version NI core).** In the rule bodies below, read "NMU" / "NSU" as the master-side / slave-side Width Bridge instance. The AMD-aligned chop / AxSize rewrite (NI-WIDTH-06/07/08/11) is pending in the Width Bridge spec.

| ID | Condition | Required behavior | Severity | ARM SVA equivalent |
|----|-----------|-------------------|----------|--------------------|
| AXI4_MST_WSTRB_REGEN | NMU is performing Upsize on a W burst (`AXI_DATA_WIDTH < NOC_DATA_WIDTH`) and accumulates AXI W beats into a wide flit | NMU MUST regenerate the wide-flit `wstrb` field per AXI byte-lane mapping: each AXI input `wstrb` byte at AXI byte position `b` maps to the wide-flit byte position `b'` derived from `awaddr` + per-beat offset + `awsize`. Bytes in the wide flit not driven by any AXI W beat (over-fetch lanes) MUST carry `wstrb=0`. Bytes outside the addressed lanes for narrow transfers (per `awsize` < `log2(AXI_DATA_WIDTH/8)`) also carry `wstrb=0`. The NSU on the receiving end relies on this mask to gate which bytes commit. | FAIL | (none) |
| AXI4_SLV_OVERFETCH_FILTER | NSU receives a wide W flit and is performing Downsize (`AXI_DATA_WIDTH > NOC_DATA_WIDTH`) or pass-through to a slave whose `AXI_DATA_WIDTH` matches the flit | NSU MUST filter the W flit's byte-lanes by the regenerated `wstrb` field: only bytes with `wstrb=1` are committed to the local AXI slave's W beat(s). Over-fetched bytes (those with `wstrb=0` from the NMU side) MUST NOT be visible to the slave. AXI4 §A3.4.4 `wstrb`-as-canonical-validity contract is preserved end-to-end. | FAIL | (none) |
