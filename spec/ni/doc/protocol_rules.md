...
| NOC_VC_ARBITER_HYBRID_RW_QOS | NMU / NSU output arbiter selects a VC for the next injection cycle when `NUM_VC > 1` | The arbitration policy is hybrid R/W × QoS weighted round-robin: VCs are partitioned into request/response subsets (per `NOC_VC_PARTITION`); within each subset the arbiter applies weighted RR with QoS-tier weighting; across subsets the arbiter alternates to prevent request/response starvation. Policy is fixed at design time; no runtime alternative. | FAIL | (none) |
