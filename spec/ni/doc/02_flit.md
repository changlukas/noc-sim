...
- **Stage 2 (routing)**: `src_id` / `dst_id` / `vc_id` / `route_par` — 路徑決策同時做 parity 檢查（route_par cover `{dst_id, last}` per AMD pg313 alignment）
...
Always-on even parity over routing-critical fields。對齊 AMD pg313 §Parity：「The NPP packet (DST ID + LAST) field is also protected by 1-bit even parity」。
...
設計用意：保護 routing 過程中 single-bit error 造成 misroute（path bit-flip 把封包導到錯誤 node 或 wormhole lock 提前 / 過晚釋放）。雖然 `flit_ecc` (SECDED) 在 endpoint 也能偵測，但 routers 不檢查 flit_ecc（per AMD §Data Integrity）— 中途 routing-critical bit-flip 必須靠 route_par 早期攔截。
...
| Property | Value |
|----------|-------|
| Coverage | `dst_id` (8b) + `last` (1b) = 9 bits at default. Aligned with AMD pg313 §Parity (NPP packet DST ID + LAST) |
...
- **Two-Layer Data Integrity** — `flit_ecc` (`FLIT_ECC_WIDTH`), `route_par` (`ROUTE_PAR_WIDTH`)
  - **Layer 1 routing protection**: 1-bit even parity over `{dst_id, last}` (9 bits at default; aligned with AMD pg313 §Parity NPP packet DST ID + LAST coverage); checked at every router hop; fatal interrupt on mismatch
